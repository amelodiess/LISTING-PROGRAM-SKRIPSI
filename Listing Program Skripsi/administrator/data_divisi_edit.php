...
if ($_GET['id'] == '') {
    ...
    $divisi_id = trim($_GET['id']);
}

function get_divisi($id)
{
    global $db;
    $query = $db->prepare('SELECT * FROM divisi WHERE id = :id');
    $query->bindValue(':id', $id, PDO::PARAM_STR);
    $query->execute();
    return $query->fetchObject();
}

function get_data_kepala($id)
{
    global $db;
    $query = $db->prepare('SELECT * FROM pengguna WHERE divisi_id IS NULL OR id = :id');
    $query->bindValue(':id', $id, PDO::PARAM_STR);
    $query->execute();
    return $query->fetchAll(PDO::FETCH_OBJ);
}

function get_data_pegawai_terpilih($id)
{
    global $db;
    $query = $db->prepare('SELECT * FROM pengguna WHERE divisi_id = :id');
    $query->bindValue(':id', $id, PDO::PARAM_STR);
    $query->execute();
    return $query->fetchAll(PDO::FETCH_OBJ);
}
...
{
    global $db;
    $query = $db->prepare('SELECT * FROM pengguna WHERE divisi_id IS NULL OR divisi_id = :id ');
    $query->bindValue(':id', $id, PDO::PARAM_STR);
    $query->execute();
    return $query->fetchAll(PDO::FETCH_OBJ);
}

$divisi_data = get_divisi($divisi_id);
$data_pegawai_terpilih = get_data_pegawai_terpilih($divisi_id);
...
if (isset($_POST['submit'])) {
    ...
    if ($query) {
        ...
        redirect(base_url('users/administrator/data_divisi_edit.php'));
    }
}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-5">
                                    <div class="form-group">
                                        ...
                                        <input required class="form-control" placeholder="Nama divisi" type="text" name="nama" value="<?=$divisi_data->nama?>">
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select required class="form-control select2" name="kepala">
                                            ...
                                            <?php foreach (get_data_kepala($divisi_data->kepala_id) as $key => $val) {
                                ...
                                <div class="col-md-6 pr-4 border-right">
                                    ...
                                    <div class="table-scroll">
                                        <table class="table table-stripped table_terpilih table-bordered mb-0">
                                            <tbody>
                                            <?php foreach ($data_pegawai_terpilih as $key => $dat) {?>
                                            <tr class="row_<?=$dat->id?>">
                                                <td width="70"><button type="button" class="btn btn-outline-danger btn-xs" onclick="destroy_row(<?=$dat->id?>)">batal</button></td>
                                                <td><?=$dat->nama?></td>
                                                <td width="150">
                                                <?=$dat->nik?>
                                                <input type="hidden" name="pegawai_terpilih[]" value="<?=$dat->id?>"/>
                                                </td>
                                            </tr>
                                            <?php }?>
                                            </tbody>
                                ...
                                <div class="col-md-6 pl-4">
                                    ...
                                    <table id="data_table" class="table table-stripped table-bordered">
                                        ...
                                        <tbody>
                                            <?php foreach (get_data_pegawai($divisi_id) as $dtp): ?>
                                                <tr class="source_<?=$dtp->id?> <?=$dtp->divisi_id == $divisi_id ? "hidden" : ""?>">
...
<script type="text/javascript">
    ...
    $(".select2").val(<?= $divisi_data->kepala_id ?>).trigger('change');  
</script>